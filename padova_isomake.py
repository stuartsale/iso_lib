...
def iso_interp(filenames, metallicity, metal_weight, output_obj, 
    ...
    logT_edges = np.arange(logT_min-logT_step/2.,
                           logT_max+logT_step,logT_step)
    logg_edges = np.arange(logg_min-logg_step/2.,
                           logg_max+logg_step,logg_step)
    ...
    for filename in filenames:
        ...
        for band in bands_dict.keys():
            if band in header_line:
                photom_cols[bands_dict[band] ] = (
                                            header_line.index(band)-1)
            

        # Read in data - need Z, age, Mi, logT, logg, r, i, Ha

        iso_data.append(np.loadtxt(filename, usecols=(Mi_col, 
                                                      logage_col,
                                                      logTe_col,
                                                      logg_col) ))
...
#            print "\r", it, interp_points.shape[1],
        ...
        if shortlist_iso.size==0:
            ...
            short_weights1 = (shortlist_weights[:,-1]/(1E-9
                             +np.power(shortlist_iso[:,2]-point[0],2.)
                             *36.
    ...
    inner_counts, xe, ye = np.histogram2d(iso_data[:,2],
                                          iso_data[:,3],
                                          bins=[logT_edges,
                                                logg_edges])
...
def padova_interpolated_isomake(directories, bands_dict,
                                output_filename, bands_ordered=None):
        ...
        for iso_file1 in iso_files_gz:
            ...
            for line in iso_data:
                ...
                if "[M/H]" in split_line:
                    metal = float(split_line[split_line.index("[M/H]")
                                             +2])
